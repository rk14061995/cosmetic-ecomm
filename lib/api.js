...
/**
 * Express mounts most routes under `/api`. Deployments often set only the origin
 * (missing `/api`), which produces exactly: GET /orders/... → 404 "Route not found".
 * Reverse proxies that strip `/api` when forwarding need the backend to also mount
 * the same router without the prefix (see `server.js` for `/orders`, `/marketing-links`).
 */
